...
export default function PastEventCard({ registration }: PastEventCardProps) {
  ...
  return (
    <Card className="overflow-hidden h-full flex flex-col opacity-85">
      ...
      <CardFooter className="pt-2 pb-4 border-t flex justify-between">
        ...
        <Link href={`/dashboard/runner/events/${event.id}`}>
          <Button size="sm" variant="outline" className="h-8">View Results</Button>
        </Link>
      </CardFooter>
    </Card>
  );
